...
from utils import safe, rsi, safe_clamp  # noqa: E402
...
def fetch_us_data():
    ...
    if spy is None or spy.empty:
        print("[US] WARNING: No data returned for SPY. Skipping US update.")
        return None, None, False
    if vix is None or vix.empty or tnx is None or tnx.empty:
        print("[US] WARNING: No data returned for VIX/TNX. Skipping US update.")
        return None, None, False
...
def fetch_tw_data():
    ...
    if twii is None or twii.empty or tsmc is None or tsmc.empty:
        print("[TW] WARNING: No data returned for TWII/TSMC. Skipping TW update.")
...
def fetch_jp_data():
    ...
    if nikkei is None or nikkei.empty:
        print("[JP] WARNING: No data returned for Nikkei. Skipping JP update.")
        return None, False
...
def fetch_eu_data():
    ...
    if stoxx is None or stoxx.empty:
        print("[EU] WARNING: No data returned for STOXX50. Skipping EU update.")
        return None, False
...
def compute_score(market_data, prefix, market_key=None):
    ...
    Returns None (instead of a bogus clamped score) if any sub-signal
    is NaN, since the score would otherwise be unreliable.
    """
    ...
    try:
        rsi_score = safe_clamp(rsi, label=f'{prefix}_RSI14')
        high_score = safe_clamp((vs_high - floor) * (100.0 / max(h_rng, 1)), label=f'{prefix}_vs_52w_high_pct')
        mom_score = safe_clamp((mom + m_rng / 2) * (100.0 / max(m_rng, 1)), label=f'{prefix}_20d_return_pct')
    except ValueError as exc:
        print(f"[SCORE] WARNING: {exc} — score for {prefix} is unreliable, returning None")
        return None
...
def update_snapshot(us_data=None, tw_data=None, tw_retail=None, global_ctx=None, usdtwd=None,
    ...
    snapshot = {
        ...
        'data_sources': {
            ...
            'scoring_method': 'Linear floor-range normalization with RSI and momentum components',
...
def update_agent_results(snapshot, us_data, tw_data, tw_retail, jp_data, kr_data, eu_data, global_ctx,
    ...
    # --- Phase 1: compute dynamic action thresholds for all markets ---
    global _CURRENT_THRESHOLDS
    _CURRENT_THRESHOLDS = {}
    for _mkt in ['us', 'tw', 'jp', 'kr', 'eu']:
        try:
            _CURRENT_THRESHOLDS[_mkt] = compute_action_thresholds(_mkt)
        except Exception as e:
            print(f"[THRESHOLDS] WARNING: could not compute thresholds for {_mkt}: {e}")
    ...
    for agent_key in ['us_agent', 'tw_agent', 'jp_agent', 'kr_agent', 'eu_agent']:
        ...
        market_key = {
            ...
            'jp_agent': 'jp', 'kr_agent': 'kr', 'eu_agent': 'eu',
        }.get(agent_key)
...
def main():
    ...
    # Each market's fetch is isolated in its own try/except so that one market's
    # failure (network error, empty yfinance response, etc.) doesn't abort the
    # whole pipeline — the failure is logged and the market falls back to its
    # existing "market closed" carry-forward path.
    if args.us:
        try:
            us_data, global_ctx, us_open = fetch_us_data()
        except Exception as e:
            print(f"[US] ERROR: fetch failed: {e}")
            us_data, global_ctx, us_open = None, None, False

    if args.tw:
        try:
            tw_data, tw_retail, usdtwd, tw_open = fetch_tw_data()
        except Exception as e:
            print(f"[TW] ERROR: fetch failed: {e}")
            tw_data, tw_retail, usdtwd, tw_open = None, None, None, False

    if args.jp:
        try:
            jp_data, jp_open = fetch_jp_data()
        except Exception as e:
            print(f"[JP] ERROR: fetch failed: {e}")
            jp_data, jp_open = None, False
        if jp_open and jp_data:
    ...
    if args.kr:
        try:
            ...
            print(f"[KR] ERROR: fetch failed: {e}")
            kr_data, kr_open = None, False
        if kr_open and kr_data:
    ...
    if args.eu:
        try:
            ...
            print(f"[EU] ERROR: fetch failed: {e}")
            eu_data, eu_open = None, False
        if eu_open and eu_data:
